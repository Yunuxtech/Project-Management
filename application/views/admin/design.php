            ...
            <div class="col-md-4">
              <div class="card">
                <div class="card-header card-header-icon card-header-rose">
                  ...
                  <h4 class="card-title"><?php if(!empty($edit_fetch_data)){ echo"Edit"; }else{ echo"Add";} ?> Design
                ...
                <div class="card-body">
                  ...
                  <form method="post" action="<?php echo base_url()?>design_update" id="LoginValidation" enctype="multipart/form-data"> 
                  <?php }else{ ?>
                  <form method="post" action="<?php echo base_url()?>design_add" id="LoginValidation" enctype="multipart/form-data"> 
                  <?php } ?>
                    <div class="row">
                      <div class="col-md-12">
                      ...
                      <div class="col-md-12">
                        <div class="form-group">
                          <label for="sub_cat_id" class="bmd-label-floating pull-right">Sub Category</label>
                           <select class="form-control" required="true" name="sub_cat_id" id="sub_cat_id" onchange="get_styles()">
                              <option value=""></option>
                      ...
                      <div class="col-md-12">
                        <div class="form-group">
                          <label for="style_id" class="bmd-label-floating pull-right">Style</label>
                           <select class="form-control" required="true" name="style_id" id="style_id">
                              <option value=""></option>
                         <?php foreach ($style_data as $key => $value) { ?>
                              <option <?php if(!empty($edit_fetch_data) && $edit_fetch_data[0]->style_id ==$value->style_id){ echo"selected";} ?> value="<?php echo $value->style_id;?>"><?php echo $value->style_name;?></option>
                         <?php } ?> 
                         </select>
                        </div>
                      </div>
                    ...
                    <div class="row">
                      <div class="col-md-12">
                        <div class="form-group">
                          <label for="design_name" class="bmd-label-floating"> Design Name*</label>
                          <input type="text" class="form-control name_type" required="true" name="design_name" <?php if(!empty($edit_fetch_data) && $edit_fetch_data[0]->design_name){ echo'value="'.$edit_fetch_data[0]->design_name.'"'; } ?> id="design_name">
                          <input type="hidden" class="form-control" required="true" name="design_id" <?php if(!empty($edit_fetch_data) && $edit_fetch_data[0]->design_id){ echo"value='".$edit_fetch_data[0]->design_id."'"; } ?> id="design_id">
                          
                        </div>
                      </div>
                      <div class="col-md-12">
                        <div class="form-group">
                          <label for="design_rate" class="bmd-label-floating"> Design Rate*</label>
                          <input type="text" class="form-control name_type" required="true" name="design_rate" <?php if(!empty($edit_fetch_data)){ echo'value="'.$edit_fetch_data[0]->design_rate.'"'; } ?> id="design_rate">
                        </div>
                      </div>
                      <div class="col-md-12 col-sm-4">
                          <h4 class="title">Design Image</h4>
                          <div class="fileinput fileinput-new text-center" data-provides="fileinput">
                              <div class="fileinput-new thumbnail">
                                <img <?php if(!empty($edit_fetch_data) && !empty($edit_fetch_data[0]->design_img)){ ?> src="<?php echo base_url()?>uploads/<?php echo $edit_fetch_data[0]->design_img; ?>"<?php }else{ ?> src="<?php echo base_url()?>assets/admin_assets/img/image_placeholder.jpg" <?php } ?>>
                              </div>
                              ...
                              <div>
                                <span class="btn btn-success btn-round btn-file">
                                  ...
                                  <input type="file" name="design_img" <?php if(!empty($edit_fetch_data) && !empty($edit_fetch_data[0]->design_img)){ }else{ ?>required <?php } ?>>
                                </span>
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary card-header-icon">
                  ...
                  <h4 class="card-title">View All Design</h4>
                </div>
                    ...
                    <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo get_value('tbl_category','cat_id',$value->cat_id,'category_name'); ?> / <?php echo get_value('tbl_sub_category','cat_id',$value->sub_cat_id,'sub_cat_name'); ?> / <?php echo get_value('tbl_styles','style_id',$value->style_id,'style_name'); ?></td>
                            <td><?php echo $value->design_rate; ?></td>
                            <td><img src="<?php echo base_url()?>uploads/<?php echo $value->design_img; ?>" class="card-avatar1"></td>
                            <td><?php echo $value->status; ?></td>
                            <td class="text-right">
                              <?php if($value->status =='YES'){ $status_val="NO";}else{ $status_val="YES";} ?>
                            <a href="<?php echo base_url()?>Admin_controller/upd_status/tbl_design/design_id/<?php echo $value->design_id;?>/status/<?php echo $status_val;?>/design" class="btn btn-link btn-warning btn-just-icon edit"
                            <?php if($value->status =='YES'){ echo"title='Turn Off Design'"; }else{ echo"title='Turn On Design'";} ?>><i class="material-icons">dvr</i></a>
                            <a href="<?php echo base_url()?>design_edit/<?php echo $value->design_id;?>" class="btn btn-link btn-danger btn-just-icon edit"><i class="material-icons">edit</i></a>
                          </td>
                        </tr>
                        <?php } ?>
                      </tbody>
...
<script>
  ...
  <script type="text/javascript">
      function get_subcategory(){
        ...
        var $url="<?php echo base_url();?>get_subcategory1";
        // alert($url);
        $.ajax({ 
          url: $url,
          type:"POST",
