...
}else
{
	...
	public function get_styles()
	{
		 $cat_id = $this->input->post("cat_id");
		 $sub_cat_id = $this->input->post("sub_cat_id");
		 $query = $this->db->get_where("tbl_styles", array('cat_id' => $cat_id,'sub_cat_id' => $sub_cat_id));
	...
	public function dir_to_array($dir, $separator = DIRECTORY_SEPARATOR, $paths = 'relative') 
{
	...
	{
		$customer_id = $this->session->userdata('customer_id');
		$query = $this->db->get_where("tbl_styles", array('customer_id' => $customer_id));
		$data['fetch_data'] = $query->result();
		$query = $this->db->get_where("tbl_category", array('status'=>'YES'));
		$data['cat_data'] = $query->result();
	...
	public function style_add()
	{
		...
		$data = array('cat_id' => $cat_id ,'sub_cat_id' => $sub_cat_id , 'style_name' => $style_name , 'customer_id' => $customer_id);

		$data['fetch_data'] = $this->crud_model->selectquery_array('tbl_styles',$data);
		if(count($data['fetch_data']) > 0)
		{
		   $this->session->set_tempdata('success', 'style Already Exsits !', 5);
		}
		else
		{
		   $datavalues = array('cat_id' => $cat_id ,'sub_cat_id' => $sub_cat_id , 'style_name' => $style_name , 'customer_id' => $customer_id);
			$data = $this->crud_model->insert('tbl_styles',$datavalues);
			$this->session->set_tempdata('success', 'style Added !', 5);
		}
		redirect(base_url('style'));
	}
	public function style_edit()
	{
		$style_id = $this->uri->segment('2');
		$customer_id = $this->session->userdata('customer_id');
		$query = $this->db->get_where("tbl_styles", array('style_id'=>$style_id,'customer_id'=>$customer_id));
		$data['edit_fetch_data'] = $query->result();
		if(!empty($data['edit_fetch_data']))
		{
			...
			$data['sub_cat_data'] = $query->result();
			$this->load->view('admin/style',$data);
		}
		else
		{
			$this->session->set_tempdata('success_mesg', 'Style Not Found !', 5);
			redirect(base_url('style'));
		}
	}
	public function style_update()
	{
		...
		if(count($data['fetch_data']) > 0)
		{
		   $this->session->set_tempdata('success_mesg', 'Style Already Exsits !', 5);
		}
		else
		{
		   $datavalues = array('cat_id' => $cat_id ,'sub_cat_id' => $sub_cat_id , 'style_name' => $style_name , 'customer_id' => $customer_id);
		   $data = $this->crud_model->update('tbl_styles',$datavalues,'style_id',$style_id);
			$this->session->set_tempdata('success_mesg', 'Style Updated !', 5);
		}
		redirect(base_url('style'));
	}
	...
	{
		$customer_id = $this->session->userdata('customer_id');
		$query = $this->db->get_where("tbl_design", array('customer_id' => $customer_id));
		$data['fetch_data'] = $query->result();
		$query = $this->db->get_where("tbl_category", array('status'=>'YES'));
		$data['cat_data'] = $query->result();
	...
	public function design_add()
	{
		...
		$sub_cat_id = $this->input->post("sub_cat_id");
		$cat_id = $this->input->post("cat_id");
		$datavalues = array('customer_id' => $customer_id,'design_rate' => $design_rate,'design_name' => $design_name,'style_id' => $style_id,'sub_cat_id' => $sub_cat_id,'cat_id' => $cat_id);

		$data['fetch_data'] = $this->crud_model->selectquery_array('tbl_design',$datavalues);
		if(count($data['fetch_data']) > 0)
		{
			$this->session->set_tempdata('success', 'Design Already Exsits !', 5);
		}
		else
		{
     		$design_img = empty($_FILES['design_img']['name']) ? '' : $_FILES['design_img']['name'];
     		if(!empty($design_img)) $design_img1 = file_upload_img($design_img, 'design_img');
     		$datavalues1 = array('customer_id' => $customer_id,'design_rate' => $design_rate,'design_name' => $design_name,'style_id' => $style_id,'sub_cat_id' => $sub_cat_id,'cat_id' => $cat_id,'design_img' => $design_img1);

		   $this->session->set_tempdata('success', 'Design Added !', 5);
		   $data = $this->crud_model->insert('tbl_design',$datavalues1);
		}
	...
	public function design_edit()
	{
		...
		$customer_id = $this->session->userdata('customer_id');
		$query = $this->db->get_where("tbl_design", array('design_id'=>$design_id,'customer_id'=>$customer_id));
		$data['edit_fetch_data'] = $query->result();
		if(!empty($data['edit_fetch_data']))
		{
			$query = $this->db->get("tbl_category");
			...
			$query = $this->db->get("tbl_sub_category");
			$data['sub_cat_data'] = $query->result();
			$query = $this->db->get("tbl_styles");
			$data['style_data'] = $query->result();
			$this->load->view('admin/design',$data);
		}
		else
		{
			$this->session->set_tempdata('success_mesg', 'Design Not Found !', 5);
			redirect(base_url('design'));
		}
	}
	...
	public function design_update()
	{
		...
		$query = $this->db->get_where("tbl_design",array('design_id' => $design_id));
     		$data['attachment_data'] = $query->result();
     		$design_img = empty($_FILES['design_img']['name']) ? '' : $_FILES['design_img']['name'];
     		if(!empty($design_img)) $design_img1 = file_upload_img($design_img, 'design_img'); else $design_img1 = empty($data['attachment_data']) ? '' : $data['attachment_data'][0]->design_img;
		$datavalues = array('customer_id' => $customer_id,'design_rate' => $design_rate,'design_name' => $design_name,'style_id' => $style_id,'sub_cat_id' => $sub_cat_id,'cat_id' => $cat_id,'design_img' => $design_img1);
		$data['edit_fetch_data'] = $this->crud_model->selectquery_array('tbl_design',$datavalues);
		if(count($data['edit_fetch_data']) > 0)
		{
		   $this->session->set_tempdata('success_mesg', 'Design Already Exsits !', 5);
		}
		else
		{
		   $datavalues1 = array('customer_id' => $customer_id,'design_rate' => $design_rate,'design_name' => $design_name,'style_id' => $style_id,'sub_cat_id' => $sub_cat_id,'cat_id' => $cat_id,'design_img' => $design_img1);
			$data = $this->crud_model->update('tbl_design',$datavalues1,'design_id',$design_id);
			$this->session->set_tempdata('success_mesg', 'Updated Design !', 5);
		}
	...
	public function product_t()
	{
		...
		$query = $this->db->get_where("tbl_category", array('status'=>'YES'));
		$data['cat_data'] = $query->result();
		$this->load->view('admin/product_t',$data);
	}
	public function get_designs()
	{
		$cat_id = $this->input->post("cat_id");
		$sub_cat_id = $this->input->post("sub_cat_id");
		$design_ids_all = explode(',', $this->input->post("design_ids_all"));
		 $query = $this->db->get_where("tbl_styles", array('cat_id' => $cat_id,'sub_cat_id' => $sub_cat_id));
     	 $data['sub_cat'] = $query->result();
     	 foreach ($data['sub_cat'] as $key => $value) {
     	 	echo"<div class='row'><div class='col-md-12'><h4 class='text-center' style='color:#ce1b57;padding-top: 10px;margin-bottom: 20px;padding-bottom:10px;background-color: rgba(0, 0, 0, 0.06);margin:10px;font-weight:bold;'>".$value->style_name."</h4></div></div>";
     	 	$query = $this->db->get_where("tbl_design", array('cat_id' => $cat_id,'style_id' => $value->style_id));
     	 	$data['design_data'] = $query->result();
     	 	echo"<div class='row'>";
     	 foreach ($data['design_data'] as $key => $value1) {
     	 	$get_check="";$get_style="";
     	 	if(in_array($value1->design_id,$design_ids_all))
     	 	{
     	 		$get_check = "checked";
     	 		$get_style = "style='border:1px solid #e91e63;'";
     	 	}
     	 	echo"<div class='col-md-3 img_m_1' ".$get_style.">
                <center><label for='inlineCheckbox".$value1->design_id."' class='check_design'><img src='".base_url()."uploads/".$value1->design_img."' ><br><input type='checkbox' value='".$value1->design_id."' name='design_id[]' class='design_ids' id='inlineCheckbox".$value1->design_id."' ".$get_check.">
                Color Botton <br><span>INR ".$value1->design_rate."</span></label>
              </center>
              </div>";
     	 }
     	 echo"</div>";
	...
	public function product_t_add()
	{
		$customer_id = $this->session->userdata('customer_id');
		$cat_id = $this->input->post("cat_id");
		$sub_cat_id = $this->input->post("sub_cat_id");
		$design_ids_all = $this->input->post("design_ids_all");
		$pro_name = $this->input->post("pro_name");
		$pro_rate = $this->input->post("pro_rate");
		$pro_price = $this->input->post("pro_price");
		$pro_details = $this->input->post("pro_details");
		$pro_img = empty($_FILES['pro_img']['name']) ? '' : $_FILES['pro_img']['name'];
     		if(!empty($pro_img)) $pro_img1 = file_upload_img($pro_img, 'pro_img');
		$datavalues1 = array('customer_id' => $customer_id,'cat_id' => $cat_id,'sub_cat_id' => $sub_cat_id,'design_id' => $design_ids_all,'pro_name' => $pro_name,'pro_rate' => $pro_rate,'pro_price' => $pro_price,'pro_details' => $pro_details,'pro_img' => $pro_img1);

		   $this->session->set_tempdata('success', 'Product Added !', 5);
		   $data = $this->crud_model->insert('tbl_product',$datavalues1);
		redirect(base_url('product_t'));
	}
	public function product_t_view()
	{
		$customer_id = $this->session->userdata('customer_id');
		$query = $this->db->get_where("tbl_product", array('customer_id' => $customer_id));
		$data['fetch_data'] = $query->result();
		$query = $this->db->get_where("tbl_category", array('status'=>'YES'));
		$data['cat_data'] = $query->result();
		$this->load->view('admin/product_t_view',$data);
	}
	public function product_t_edit()
	{
		$pro_id = $this->uri->segment('2');
		$customer_id = $this->session->userdata('customer_id');
		$datavalues = array('customer_id' => $customer_id , 'pro_id' => $pro_id);
		$data['edit_fetch_data'] = $this->crud_model->selectquery_array('tbl_product',$datavalues);
		if(count($data['edit_fetch_data']) > 0)
		{
			$query = $this->db->get("tbl_category");
			$data['cat_data'] = $query->result();
		...
		{
		   $this->session->set_tempdata('success', 'Product Wrongly select !', 5);
		   redirect(base_url('product_t_view'));
		}
	}
	public function product_t_update()
	{
		$customer_id = $this->session->userdata('customer_id');
		$cat_id = $this->input->post("cat_id");
		$sub_cat_id = $this->input->post("sub_cat_id");
		...
		$query = $this->db->get_where("tbl_product",array('pro_id' => $pro_id));
     	$data['attachment_data'] = $query->result();
     	$pro_img = empty($_FILES['pro_img']['name']) ? '' : $_FILES['pro_img']['name'];
     	if(!empty($pro_img)) $pro_img1 = file_upload_img($pro_img, 'pro_img'); else $pro_img1 = empty($data['attachment_data']) ? '' : $data['attachment_data'][0]->pro_img;
     	$datavalues1 = array('customer_id' => $customer_id,'cat_id' => $cat_id,'sub_cat_id' => $sub_cat_id,'design_id' => $design_ids_all,'pro_name' => $pro_name,'pro_rate' => $pro_rate,'pro_price' => $pro_price,'pro_details' => $pro_details,'pro_img' => $pro_img1);
		$data = $this->crud_model->update('tbl_product',$datavalues1,'pro_id',$pro_id);
			$this->session->set_tempdata('success_mesg', 'Updated Product !', 5);
			// $this->db->last_query();
			// die();
		redirect(base_url('product_t_view'));
	}
